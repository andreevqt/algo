...
/*
 * https://contest.yandex.ru/contest/25597/run-report/74303268/
 *
 * -- ПРИНЦИП РАБОТЫ --
 ...
 * Создадим массив dp[2][m + 1]. В dp мы будем хранить расстояние между подстроками i, j. Инициализируем 1 строку числами 0...m и 1 столбец 0...n - 
 ...
 * Переход динамики:
 *  1) если символы i - 1, j - 1 равны, то dp[i % n ][j] = dp[(i - 1) % n][j - 1] (влево вверх по диагонали на 1 клеточку)
 *  2) иначе min(dp[i % 2][j - 1], dp[(i - 1) % 2][j], dp[(i - 1) % 2][j - 1]) + 1 - минимальное значение между символом слева и символом сверху + 1
 * Порядок обхода: от начала до конца каждой строки
 * Ответ будет находится в dp[n % 2][m], где n, m длины строк
 * -- ВРЕМЕННАЯ СЛОЖНОСТЬ --
 * Средний случай: O(N * M) - где N - длина первой строки, M - длина второй
  * -- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
 * O(M) - хранение массива dp, где M - длина второй строки
 * */
